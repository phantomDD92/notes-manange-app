...
function addEntry(folder, inFile, outFile) {
  const exports =
    inFile === "PrimeVue.js" || folder === "passthrough/tailwind"
      ? "named"
      : "auto";
  const useCorePlugin = Object.keys(GLOBAL_COMPONENT_DEPENDENCIES).some(
    (d) => d.replace("primevue/", "") === folder
  );
...
function addSFC() {
  fs.readdirSync(path.resolve(__dirname, "./components/lib"), {
    withFileTypes: true,
  })
    .filter((dir) => dir.isDirectory())
    .forEach(({ name: folderName }) => {
      fs.readdirSync(
        path.resolve(__dirname, "./components/lib/" + folderName)
      ).forEach((file) => {
...
function addIcon() {
  fs.readdirSync(path.resolve(__dirname, "./components/lib/icons"), {
    withFileTypes: true,
  })
    .filter((dir) => dir.isDirectory())
    .forEach(({ name: folderName }) => {
      fs.readdirSync(
        path.resolve(__dirname, "./components/lib/icons/" + folderName)
      ).forEach((file) => {
        if (/\.vue$/.test(file)) {
...
function addStyle() {
  fs.readdirSync(path.resolve(__dirname, "./components/lib"), {
    ...
    .forEach(({ name: folderName }) => {
      try {
        fs.readdirSync(
          path.resolve(__dirname, "./components/lib/" + folderName + "/style")
        ).forEach((file) => {
          if (/\.js$/.test(file)) {
